    ...
    with torch.no_grad():
        for batch_num, (inputs, paths) in enumerate(tqdm(loader, desc='Predict')):
            ...
            for i, image_name in enumerate(paths):
                ...
                full_mask = np.zeros((original_height, original_width,3))
...
if __name__ == '__main__':
    ...
    if args.fold == -1:
        ...
        model = get_model(str(Path(args.model_path).joinpath('model_{fold}_{net}_{we}.pt'.format(fold=args.fold,net=args.model_type,we=args.jaccard_weight))),
                          model_type=args.model_type, problem_type=args.problem_type)
