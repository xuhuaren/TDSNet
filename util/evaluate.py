...
if __name__ == '__main__':
    ...
    if args.problem_type == 'binary':
        ...
        for instrument_id in tqdm([5, 10, 13]):
            for file_name in (
                            Path(args.train_path) / (
                                    'seq_' + str(instrument_id)) / 'instruments_masks').glob(
                '*'):
                y_true = cv2.imread(str(file_name), 0)

                pred_file_name = Path(args.target_path) / (
                    'seq_' + str(instrument_id)) / file_name.name
